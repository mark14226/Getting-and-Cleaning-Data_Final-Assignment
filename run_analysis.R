...
# Download data for assignmenet and put into working directory
...
alldata2 <- alldata[, c(1, 2, grep("mean()", colnames(alldata), fixed = TRUE), grep("std()", colnames(alldata), fixed = TRUE))]
...
# This data frame, alldataCast, contains the mean values for each variable, calculated by both Subject (1-30) and Activity
alldataCast
